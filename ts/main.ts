...
utility = {
  totez: (m : string) : number => parseInt(m) / 1000000,
  mutez: function (tz: string) : string {
    return new BN(new BN(tz).toFixed(6)).multipliedBy(1000000).toString();
  },
  b58cencode: function (payload: Uint8Array, prefix: Uint8Array) {
    const n = new Uint8Array(prefix.length + payload.length);
    n.set(prefix);
    n.set(payload, prefix.length);
    return library.bs58check.encode(new Buffer(n, 'hex'));
  },
  b58cdecode: function(enc: string, prefix: Uint8Array) { return library.bs58check.decode(enc).slice(prefix.length)},
  buf2hex: function (buffer: Uint8Array): string {
  ...
  hex2buf: function(hex: string): Uint8Array {
    let buffArr: number[] = [];

    let matchArr: RegExpMatchArray | null = hex.match(/[\da-f]{2}/gi);
    if (matchArr != null) { 
      buffArr = matchArr.map(function (h: string) {
  ...
  hexNonce: function (length: number) {
    var chars = '0123456789abcedf';
    var hex = '';
    while (length--) hex += chars[(Math.random() * 16) | 0];
    return hex;
  },
  mergebuf : function(b1: Uint8Array,b2: Uint8Array){
    var r = new Uint8Array(b1.length+b2.length);
    r.set(b1);
    r.set(b2, b1.length);
    return r;
  },
  sexp2mic: function me(mi : string) {
  ...
  formatMoney: function(num: number, dec: number, decPoint: string = ".", thousSep: string = ",") {
    if (isNaN(dec) || dec < 0) { dec = 2; }
    let intString = Math.abs(num).toFixed(0);
    let decString = num.toFixed(dec).split(".")[1];
    let retString: string = "";
    for (let i = intString.length - 1; i >= 0; i--) {
      retString = intString[i] + retString;
      if ((intString.length - i) % 3 == 0 && i > 0) { retString = thousSep + retString; }
    }
    return (num < 0 ? "-" : "") + retString + (dec > 0 ? decPoint + decString : "");
  },
  ml2tzjson : function (mi: string) {},
  tzjson2arr : function (s: any) : any[] { return [] },
  mlraw2json : function (mi: string) { return [] },
  mintotz : function (m : string) : number { return 0; },
  tztomin : function (tz: any) : string { return ""; },
},
//TODO: Add p256 and secp256k1 cryptographay
ecrypto = {
  extractEncryptedKeys : function(esk: string, password: string){
  ...
  extractKeys : function(sk: string){
  ...
  checkAddress: function (a: string) {
    try {
  ...
  generateKeys: function (m: string, p: string) {
...
let node = {
  ...
  debugMode: false,
  async: true,
  setDebugMode: function (t: boolean) {
    node.debugMode = t;
  },
  setProvider: function (u: string) {
    node.activeProvider = u;
  },
  resetProvider: function () {
    node.activeProvider = defaultProvider;
  },
  query: function (e : string, o?: object, t?: string) {
...
rpc = {
  ...
  call: function (e: string, d: object): Promise<{}> {
    return node.query(e, d);
  },
  sendOperation: function (from: string, operation, keys) {
    ...
    .then((f) => {
      var opbytes = f;
      opOb.protocol = this.head.protocol;
      if (keys.sk === false) {
        return {
          opOb : opOb,
          opbytes : opbytes
  ...
  transfer: function (from, keys, to, amount, fee, parameter, gasLimit: string, storageLimit: string) {
  ...
  runCode(code, amount, input, storage, trace: boolean) {
...
contract = {
  hash : function(operationHash: string, ind: number) : string {
  ...
  send: function (contract: string, from, keys, amount, parameter, fee, gasLimit, storageLimit) {      
  ...
  balance: function (contract: string) {
    return rpc.getBalance(contract);
  },
  storage: function (contract: string) {
    return new Promise(function (resolve, reject) {
  ...
  watch: function (cc: string, timeout: number, cb: (...args : any[]) => any) {
    let storage: string[] = [];
    const ct = function () {
